...
public class RequestContent implements HttpRequestInterceptor {
    ...
    public void process(final HttpRequest request, final HttpContext context)
        ...
        if (request instanceof HttpEntityEnclosingRequest) {
            if (this.overwrite) {
                request.removeHeaders(org.apache.httpcopy.protocol.HTTP.TRANSFER_ENCODING);
                request.removeHeaders(org.apache.httpcopy.protocol.HTTP.CONTENT_LEN);
            } else {
                if (request.containsHeader(org.apache.httpcopy.protocol.HTTP.TRANSFER_ENCODING)) {
                    throw new ProtocolException("Transfer-encoding header already present");
                }
                if (request.containsHeader(org.apache.httpcopy.protocol.HTTP.CONTENT_LEN)) {
            ...
            if (entity == null) {
                request.addHeader(org.apache.httpcopy.protocol.HTTP.CONTENT_LEN, "0");
                return;
            }
            ...
            if (entity.isChunked() || entity.getContentLength() < 0) {
                ...
                request.addHeader(org.apache.httpcopy.protocol.HTTP.TRANSFER_ENCODING, org.apache.httpcopy.protocol.HTTP.CHUNK_CODING);
            } else {
                request.addHeader(org.apache.httpcopy.protocol.HTTP.CONTENT_LEN, Long.toString(entity.getContentLength()));
            }
