...
public class ResponseConnControl implements HttpResponseInterceptor {
    ...
    public void process(final HttpResponse response, final HttpContext context)
        ...
        if (status == HttpStatus.SC_BAD_REQUEST ||
            ...
            response.setHeader(org.apache.httpcopy.protocol.HTTP.CONN_DIRECTIVE, org.apache.httpcopy.protocol.HTTP.CONN_CLOSE);
            return;
        }
        final Header explicit = response.getFirstHeader(org.apache.httpcopy.protocol.HTTP.CONN_DIRECTIVE);
        if (explicit != null && org.apache.httpcopy.protocol.HTTP.CONN_CLOSE.equalsIgnoreCase(explicit.getValue())) {
        ...
        if (request != null) {
            final Header header = request.getFirstHeader(org.apache.httpcopy.protocol.HTTP.CONN_DIRECTIVE);
            if (header != null) {
                response.setHeader(org.apache.httpcopy.protocol.HTTP.CONN_DIRECTIVE, header.getValue());
