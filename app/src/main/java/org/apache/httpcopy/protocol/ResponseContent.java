...
public class ResponseContent implements HttpResponseInterceptor {
    ...
    public void process(final HttpResponse response, final HttpContext context)
        ...
        if (this.overwrite) {
            response.removeHeaders(org.apache.httpcopy.protocol.HTTP.TRANSFER_ENCODING);
            response.removeHeaders(org.apache.httpcopy.protocol.HTTP.CONTENT_LEN);
        } else {
            if (response.containsHeader(org.apache.httpcopy.protocol.HTTP.TRANSFER_ENCODING)) {
                throw new ProtocolException("Transfer-encoding header already present");
            }
            if (response.containsHeader(org.apache.httpcopy.protocol.HTTP.CONTENT_LEN)) {
        ...
        if (entity != null) {
            ...
            if (entity.isChunked() && !ver.lessEquals(HttpVersion.HTTP_1_0)) {
                response.addHeader(org.apache.httpcopy.protocol.HTTP.TRANSFER_ENCODING, org.apache.httpcopy.protocol.HTTP.CHUNK_CODING);
            } else if (len >= 0) {
                response.addHeader(org.apache.httpcopy.protocol.HTTP.CONTENT_LEN, Long.toString(entity.getContentLength()));
            }
            // Specify a content type if known
            if (entity.getContentType() != null && !response.containsHeader(
                    org.apache.httpcopy.protocol.HTTP.CONTENT_TYPE )) {
                response.addHeader(entity.getContentType());
            }
            // Specify a content encoding if known
            if (entity.getContentEncoding() != null && !response.containsHeader(
                    org.apache.httpcopy.protocol.HTTP.CONTENT_ENCODING)) {
            ...
            if (status != HttpStatus.SC_NO_CONTENT
                ...
                response.addHeader(org.apache.httpcopy.protocol.HTTP.CONTENT_LEN, "0");
            }
        }
    }
